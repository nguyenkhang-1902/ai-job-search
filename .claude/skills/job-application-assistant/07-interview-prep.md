...
### RAG Chatbot - Internal Knowledge Base (LLM systems, security/RBAC, AI-assisted development)
**Source:** CV - Independent Project, Jun 2026-Present
**What happened:** Built a role-based-access-controlled RAG chatbot for internal knowledge bases, running fully on-premises, with semantic caching, streaming responses, audit logging, and a citation-hallucination guardrail.
**Why it matters:** Answers questions about LLM system design, security/access control in AI systems, testing/evaluating non-deterministic systems, and how AI coding tools fit into your workflow.
**S/T/A/R:**
- **Situation:** Self-initiated portfolio project for Data Engineer/AI Engineer applications, built solo with AI pair-programming (Claude Code, following TDD) — worth naming explicitly in interviews as working like a senior dev doing step-by-step review, not writing 100% of the code by hand. Domain: enterprise internal documentation Q&A, using GitLab's open-source Handbook (~2000 pages, structured by real departments — Engineering, HR, Finance, Legal, Sales, Marketing) plus some self-authored Vietnamese documents to prove the pipeline handles both languages.
- **Task:** Build a production-grade RAG system, not a demo, around three required pillars: (1) RBAC — an employee in one department must never see another department's documents, even via prompt injection; (2) data privacy — fully on-prem, no external LLM API calls; (3) production concerns — observability, semantic caching, streaming responses, audit logging, and an eval pipeline for RAG quality.
- **Action:** Chose the stack deliberately rather than defaulting: BAAI/bge-m3 embeddings for multilingual VN/EN support; Qdrant as the vector DB for native payload filtering (needed for RBAC), self-hosting, and solid LangChain support — evaluated against Pinecone (cloud-only), Weaviate/Milvus (too heavy to operate solo in 2-3 weeks), and Chroma (weaker filtering). Used LangChain (LCEL) for the prompt chain but called `qdrant-client` directly for retrieval instead of hiding behind LangChain's retriever wrapper, so the RBAC filter stays explicit and independently testable. Local LLM via LM Studio (OpenAI-compatible endpoint, currently Qwen 4B Instruct); FastAPI backend with SSE streaming; Postgres for users/roles/audit log (needed ACID guarantees); Redis for semantic caching; Streamlit UI. Solved a citation/hallucination risk: the small local model wasn't reliable enough to force structured JSON output (Pydantic schema) while still streaming tokens, so the LLM only marks inline citations as `[chunk_id]`, while the actual source list renders directly from retrieval-step metadata (known before the LLM is even called) — after streaming, a guardrail re-scans the answer and flags any `chunk_id` that doesn't match a retrieved chunk as a hallucinated citation. Treated RBAC as the highest-risk part: the filter lives server-side in the Qdrant payload layer, built from the role in a verified JWT (never trusting a role from the request body/query params), with dedicated tests simulating a role-A user probing for role-B content, including prompt-injection attempts to bypass the filter through the prompt layer. For the semantic cache, didn't reuse a 0.92 similarity threshold from an unrelated earlier project, since a copied threshold risks domain-specific false positives (e.g. "leave policy" vs. "resignation policy" sit close in vector space but mean opposite things) — tuned the threshold empirically for this Q&A domain instead. Hit and fixed real bugs along the way: LangChain 1.3.x moved `RecursiveCharacterTextSplitter`'s import path; the test suite's `FakeListChatModel` streamed character-by-character, requiring a token-joining helper before assertions — which doubles as the same logic a real SSE client needs to consume the stream correctly.
- **Result:** 43/43 tests passing against mocked infrastructure (in-memory Qdrant, fakeredis, fake LLM), covering RBAC-bypass and prompt-injection attempts plus hallucinated-citation detection. Currently moving to validation against live infrastructure (real Qdrant/Postgres/Redis/LM Studio) and already diagnosed and worked around a real CUDA out-of-memory error offloading Qwen 4B to a 4GB-VRAM GPU by reducing `n_gpu_layers`. Honest current status: solid on mocked tests, live-infra validation in progress — not yet claiming production deployment.
- **Use for:** "Tell me about a RAG system you built", "How do you handle security/access control in an LLM system?", "How do you use AI coding tools (Claude Code) in your own workflow?", "How do you test or evaluate a non-deterministic system?"

### BHXH Tracking & Reporting Tool - Hansoll Vina (full-stack ownership, real production bug, working solo)
**Source:** CV - Human Resources Intern, Hansoll Vina, May 2026-Jun 2026
**What happened:** Designed and built an internal tool (Streamlit, FastAPI, SQLite) that replaced HR's manual Excel-based tracking of employee social-insurance (BHXH/BHTN) participation history, with automated monthly ingestion, carry-forward logic, and report exports.
**Why it matters:** Answers questions about end-to-end ownership of a real internal tool (not a portfolio exercise), working solo under a live business need, and handling a production bug responsibly.
**S/T/A/R:**
- **Situation:** Internal tool for Hansoll Vina's HR department (May-Jun 2026, built solo), replacing a manual Excel-based tracking process. Each month HR uploads a labor data file, and the system needed to automatically update each employee's BHXH/BHTN (social/unemployment insurance) participation history and generate reports — a real operational need from the company, not an academic exercise.
- **Task:** Build three things: (1) monthly data upload and validation, auto-detecting the month/year in the file and auto-backing up the database before any overwrite; (2) three Excel report types matching the company's required format exactly (per-employee history by ID, active-employee list, terminated-employee list); (3) an automatic carry-forward mechanism so an employee missing from a given month's file keeps their prior status instead of disappearing from the system.
- **Action:** Built on Streamlit (frontend), FastAPI (backend), SQLite (database) — fully internal, no external BHXH VN API, since HR uploads the Excel file by hand each month. Column names were inconsistent across months (mixed Vietnamese/English, many variants), so built a centralized mapping table normalizing roughly 40 column-name variants to one field. The month/year wasn't in a fixed position in the file, so used a 3-tier fallback: a fixed marker cell, then a header search for cells containing "Tháng", then a broad area scan. The carry-forward logic got complex for employees on special leave-status codes — had to look back up to 10 months to find the nearest salary figure as a basis, to avoid double-counting or corrupting insurance data. Hit a real bug in production: a special status label was mistakenly overwritten as "Terminated" in a report — fixed it and wrote an automated regression test to guard against recurrence. Packaged the app with an embeddable Python distribution so non-technical HR staff wouldn't need to install Python themselves.
- **Result:** Two pilot users in the HR department; the system runs 100% to the company's actual requirements, and the caught status-overwrite bug hasn't recurred since the regression test was added. Currently being refactored into smaller modules for maintainability — an honest in-progress state, not oversold as finished.
- **Use for:** "Tell me about a project you owned end-to-end", "Tell me about a bug you caused/fixed in production", "Describe building something for a non-technical user", "Tell me about a time you worked independently on a real business problem."
